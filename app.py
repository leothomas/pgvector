...
BUCKET_NAME = "similarity-search-deploy-ecs-production"
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        task_definition = aws_ecs.FargateTaskDefinition(
            ...
            cpu=16384,  # 16 Gb
            memory_limit_mib=122880,  # 120 Gb (max)
        )
        ...
        task_definition.add_to_task_role_policy(
            aws_iam.PolicyStatement(
                ...
                resources=[rds.secret.secret_arn],
            )
        )
        ...
        ecs_service = aws_ecs.FargateService(
            self,
            "fargate-service",
            cluster=aws_ecs.Cluster(self, "ecs-cluster", vpc=vpc),
            task_definition=task_definition,
        )

        # enable ingress from
        rds.connections.security_groups[0].add_ingress_rule(
            ecs_service.connections.security_groups[0], aws_ec2.Port.tcp(5432)
        )

        crt_ip = requests.get("https://api.ipify.org").content.decode("utf8")
        rds.connections.security_groups[0].add_ingress_rule(
            aws_ec2.Peer.ipv4(f"{crt_ip}/32"), aws_ec2.Port.tcp(5432)
        )

        db_secrets_output = CfnOutput(
            self, "database-secrets-arn", value=rds.secret.secret_arn
        )

        task_definition_output = CfnOutput(
            self, "task-definition-arn", value=task_definition.task_definition_arn
        )


pg_vectors_test_stack = pgVectorsTest(app, "pg-vectors-similarity-search-test", env=env)
